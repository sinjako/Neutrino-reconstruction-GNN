...
if __name__ == '__main__':
    ...
    model =dynedgeEdgepool(c=3,k=[8,4,4,4]).to(device)
    modelinfo="dynedgeEdgepool_c=3,k=8_4_4_4_lr=0.005" # DO NOT USE PARENTHESIS LINUX NO LIKE
    # model=simpedge().to(device)
    # modelinfo="simpedge"
    
    optimizer = torch.optim.Adam(model.parameters(), lr=0.2*1e-3, eps= 1e-3)
    ...
    letarget_list="energy_log10,time,vertex_x,vertex_y,vertex_z,direction_x,direction_y,direction_z,azimuth,zenith,pid,interaction_type,muon_track_ngth,stopped_muon"
    targetvar=8
