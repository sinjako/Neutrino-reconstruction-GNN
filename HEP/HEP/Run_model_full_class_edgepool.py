    ...
    for epoch in tqdm(range(epochs)):
        ...
        for k in range(train_batches):
            with torch.enable_grad():
                ...
                vals=Pidclass(batch.y[:,targetvar])

                optimizer.zero_grad()
                output=model(batch)
               # print (output.size(),vals.size())
                #print (output.size(),k)
                #print ("donezo",k)
                loss=lossfunction(output,vals)
                loss.backward()
                optimizer.step()
...
if __name__ == '__main__':
    ...
    model =dynedgeEdgepool(c=3,k=[8,8,8,8],mode='pid').to(device)
    modelinfo="dynedgeEdgepool_c=3,k=8_8_8_8_full_pid_lr0002" # DO NOT USE PARENTHESIS LINUX NO LIKE
    # model=simpedge().to(device)
    # modelinfo="simpedge"
    
    optimizer = torch.optim.Adam(model.parameters(), lr=0.002*1e-3, eps= 1e-3)
    ...
    targetvar=10 # determines which label we run over.
