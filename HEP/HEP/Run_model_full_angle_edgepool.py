...
def Predict(model,prediction_graphs,workers,pred_mini_batches,batch_size,currfolder,device,targetvar):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            truth           = data.y[:,targetvar].unsqueeze(1).detach().cpu().numpy()
        ...
        result.columns  = ['event_no','Angle','Mises1_pred','Mises2pred','Mises3pred']
...
if __name__ == '__main__':
    ...
    model =dynedgeEdgepool(c=3,k=[8,8,8,8],mode='angle').to(device)
    modelinfo="dynedgeEdgepool_c=3,k=8_8_8_8_full_angle_lr0002" # DO NOT USE PARENTHESIS LINUX NO LIKE
    # model=simpedge().to(device)
    # modelinfo="simpedge"
    
    optimizer = torch.optim.Adam(model.parameters(), lr=0.004*1e-3, eps= 1e-3)
    ...
    val_graphs=list()
    train_graphs=list()
    Nfiles=330 # chosen so the last partial file is excluded, choose 32 and 26 for 500k nodes
    valfilesind=300 # where you switch to loading into validation graph
    graph_job=4
    train_batches=4800 ## THESE ARE GAINED FROM count_batches.py
    val_batches=464## THESE ARE GAINED FROM count_batches.py
    ...
    targetvar=9 # determines which label we run over.
    lossfunction=VonMisesSineCosineLoss
   ...
   # for i in range(graph_job):
    ...
    evalbatches=val_batches
    print (len(eval_graphs))
    Predict(model,eval_graphs,workers,evalbatches,batch_size,dirstring,device,targetvar) # not commenting this out takes both a fuckton of space AND TIME
    state={
    'modelinfo': modelinfo,
