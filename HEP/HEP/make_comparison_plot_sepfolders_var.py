...
###############################################
    ...
    psad=host.hist(residlist,num_bins,label=labels,histtype='step',range=(0,rmax),align="left")
    ...
    #         means.append(np.mean(E_pred[index]))
    #         means_E.append(np.mean(E[index]))
    #         var.append(np.mean(variances[index]))
    #     varlist.append(1/np.sqrt(var))
    #     meanslist.append(means)
    #     means_E_list.append(means_E)
    ...
    # host2.plot(means_E,means_E)
    # for i in range(plotlength):
    #     host2.errorbar(means_E_list[i],meanslist[i],varlist[i],linestyle='dotted',fmt = 'o',markersize=3,capsize = 8,label=model_name[i])
    # num_bins = 20
    # n, bins, patches = par2.hist(E, num_bins, facecolor='blue', alpha=0.1,label = model_name)   # no need for this in loop since this is histogram of events 
...
def grabdata(path,transformer):
    ...
    data["E_pred_var"]=transformer["truth"][transform].inverse_transform(data[["E_pred_var"]])
    return data
...
if figname != "":
    ...
    for fig in my_cool_figures:
        ...
        if count==6:
            currstring="variances__%s_%s"%(firstsave,secondsave)
            fig.savefig(dirstring+firstsave+"//"+currstring)
            fig.savefig(dirstring+secondsave+"//"+currstring)
