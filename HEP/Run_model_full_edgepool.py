...
if __name__ == '__main__':
    ...
    modelinfo="dynedgeEdgepool_c=3,k=8_8_8_8_full_lr0.002_energy_new" # DO NOT USE PARENTHESIS LINUX NO LIKE
    ...
    for state in optimizer.state.values():
        for k, v in state.items():
            if isinstance(v, torch.Tensor):
                state[k] = v.cuda()
    currentset="dev_numu_train__l5_retro_001/"
    ...
    graph_job=4
    train_batches=4800 ##7 THESE ARE GAINED FROM count_batches.py,batchsize 1000= 4800 for normal, 5248 for upgrade
    val_batches=464## THESE ARE GAINED FROM count_batches.py, batchsize 1000= 464 for normal, 480 for upgrade
