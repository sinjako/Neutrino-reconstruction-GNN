...
########################################
    ...
    ### MAKING FIRST PLOT    
        ...
        for i in range(plotlength):
            host2.errorbar(means_E[i],means[i],error[i],linestyle='dotted',fmt = 'o',capsize = 15)
...
path_to_folder="D:\Schoolwork\Masters\Script\Savefolder\\17-05-2021-08-50-56-dynedge(k=[4,4,4,4])"
data=grabdata(path,transformer)
my_cool_figures = MakePerformancePlots(data,"dynedge(k=[4,4,4,4]))")
count=0
