...
class IceGraph(Dataset):
    ...
    def __init__(self, data,features,truth,labels,Events,**kwargs):
        """
    ...
    def read(self):
            def make_graph(event,eventno):
                ...
                for i in range(Npulse):
                    "loop that generates the events adjacency matrix"
                    Distidx=Icedist(event,event.iloc[0]).nsmallest(self.N_neighbors+1)#gets indices in event, keep selfelement so the diagonal is preserved
                    Distidx=Distidx.index -idxstart #subtract idxstart to be able to put into a
                    a[i,Distidx]=1
...
data=IceGraph(features,params,truth,targets,Events)
